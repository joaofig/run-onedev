...
#       Configuration GUI.
### BEGIN INIT INFO
# Provides: onedev_enable-internal-login
...
# Default-Stop: 0 1 6
# Short-Description: OneDev Enable Internal Login
# Description: OneDev Enable Internal Login
### END INIT INFO
...
APP_NAME="onedev_enable-internal-login"
APP_LONG_NAME="OneDev Enable Internal Login"
...
prepAdditionalParams() {
    ...
    if [ "X${PASS_THROUGH}" = "Xapp_args" -o "X${PASS_THROUGH}" = "Xboth" ] ; then
        if [ "X${PASS_THROUGH}" = "Xapp_args" ] ; then
            ADDITIONAL_PARA="wrapper.logfile.loglevel=NONE wrapper.console.title='OneDev Enable Internal Login' wrapper.description='OneDev enable-internal-login' -- enable-internal-login"
...
stopit() {
    ...
    if [ $? -eq 0 ] ; then
        ...
        # We cannot predict how long it will take for the wrapper to
